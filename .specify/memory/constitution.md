...
Modified Principles : II. 技术栈约束 — UI 组件库从 Microsoft Fluent UI for Blazor 替换为 Tailwind CSS v4
Added Sections  : N/A
Removed Sections: N/A
Templates Updated:
  ✅ .specify/memory/constitution.md — updated (this file)
  ✅ .specify/templates/plan-template.md — Constitution Check II 更新为 Tailwind CSS，版本引用 → v2.0.0
  ✅ .specify/templates/tasks-template.md — 版本引用更新至 v2.0.0
  ✅ .github/copilot-instructions.md — Tech Stack 表格与 UI 约定全面更新
  ⚠ specs/001-core-infra-mvp/spec.md — FR-006/FR-007 的 Fluent UI 引用待手动更新
  ⚠ specs/main/plan.md — Fluent UI 技术决策引用待手动更新
  ⚠ specs/main/tasks.md — 建议重新执行 /speckit.tasks 重新生成（含 Fluent UI 相关任务）
  ⚠ AnyDrop/Components/_Imports.razor — @using Microsoft.FluentUI.AspNetCore.Components 待移除
  ⚠ AnyDrop/AnyDrop.csproj — FluentUI NuGet 包待移除，Tailwind CSS 集成方式待确认
Deferred TODOs  : none
-->
...
- **UI 组件**：Tailwind CSS v4（通过 CDN `<script>` 标签快速集成，或通过 npm + PostCSS 构建链
  用于生产环境）；禁止使用 Bootstrap、Microsoft Fluent UI for Blazor 或其他 CSS 组件框架
- **图标**：优先使用 Heroicons（内联 SVG 方式），禁止引入 FluentUI 图标包
- **样式规范**：MUST 通过 Tailwind 工具类（utility classes）实现页面样式；布局使用 Tailwind
  Flexbox/Grid 工具类（`flex`、`grid`、`gap-*` 等）；禁止内联 `style` 属性或使用硬编码颜色值；
  自定义样式 MUST 在 `wwwroot/app.css` 中以 `@layer` 或 CSS 变量方式组织
- **数据库**：SQLite，通过 EF Core 操作；禁止裸 SQL 字符串拼接，MUST 使用参数化查询
- **实时通信**：SignalR Hub（服务端广播）+ Blazor ComponentBase 双向绑定（表单/状态同步）；
  不得引入第三方实时通信库
- **移动端 API**：使用 .NET 10 Minimal API 实现 RESTful HTTP 接口，供手机 App 调用；
  禁止使用 MVC Controller 风格，所有端点 MUST 通过 `app.Map*` 在 `Program.cs` 或
  专用 `Api/` 模块中注册
...
### VII. RESTful API — .NET Minimal API 优先

AnyDrop 需提供一套标准 HTTP API，供将来的手机 App（iOS/Android）及第三方客户端调用。

- **实现方式**：使用 .NET 10 Minimal API（`app.MapGet/MapPost/MapPut/MapDelete`）；
  MUST NOT 引入 MVC Controller（`[ApiController]`）模式，保持单体简洁性。
- **路由规范**：遵循 RESTful 标准，路径使用复数名词小写 kebab-case，例如：
  `GET /api/messages`、`POST /api/messages`、`GET /api/files/{id}`、`DELETE /api/files/{id}`。
- **版本策略**：API 根路径前缀为 `/api/v1/`；当破坏性变更发生时递增主版本号。
- **响应格式**：统一使用 JSON（`application/json`）；成功/失败均返回结构化响应体，
  包含 `success`、`data`、`error` 字段。
- **认证**：API 端点 MUST 复用与 Web UI 相同的认证机制（Bearer Token 或 Cookie），
  不得单独实现绕过认证的快捷通道。
- **代码组织**：API 端点定义 MUST 提取至 `Api/` 目录下的扩展方法文件（如 `MessageEndpoints.cs`），
  通过 `app.MapMessageEndpoints()` 在 `Program.cs` 中注册，保持 `Program.cs` 简洁。
- **文档**：MUST 集成 OpenAPI（`app.MapOpenApi()`），开发环境自动暴露 `/openapi/v1.json`
  及 Swagger UI，供 App 开发者查阅接口契约。
- **服务复用**：API 端点 MUST NOT 重复实现业务逻辑，MUST 注入并调用 `Services/` 层的服务接口，
  与 Blazor 组件共享同一套业务逻辑。

## 架构约束

- **通信契约**：SignalR Hub 负责服务端向所有已连接客户端的多播推送；
  Blazor Server 双向绑定仅用于当前会话的状态同步，不可替代多端广播
- **API 与 Blazor 共存**：Minimal API 路由（`/api/*`）与 Blazor Server 路由在同一进程内共存；
  两者共享 DI 容器中的 Service 实例，API 端点不得绕过服务层直接访问数据库
